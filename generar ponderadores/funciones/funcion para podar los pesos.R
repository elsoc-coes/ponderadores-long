...
trimpesos_unev<- function(base,abajo,arriba,peso){

  
  high<-quantile(getElement(base,peso),seq(0,1,by=.01),na.rm = TRUE)[1+arriba]
  low<- quantile(getElement(base,peso),seq(0,1,by=.01),na.rm = TRUE)[1+abajo]
