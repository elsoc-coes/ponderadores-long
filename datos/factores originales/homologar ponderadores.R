...
pond_16 <- read_dta("datos/factores originales/2016/ponderadores_coes.dta")%>%
            rename(idencuesta=id,pd_diseno=W1,pd_nr=W2,pd_sexo=W4,pd_elsoc=PonderadorW4)%>%
            select(idencuesta,pd_elsoc,pd_diseno,pd_nr,pd_sexo)%>%
            mutate(ola=1,
                   muestra=1,
        #Agregar ID cambiado
...
pond_17 <- read_dta("datos/factores originales/2017/factores de expansion ELSOC 2017.dta")%>%
            rename(pd_diseno=W,pd_nr=W1,pd_sexo=W3,pd_elsoc=ponderador1)%>%
            select(idencuesta,pd_elsoc,pd_diseno,pd_nr,pd_sexo)%>%
            mutate(ola=2,
                   muestra=1)
...
pond_19_m1 <- read_dta("datos/factores originales/2019/factores_Seguimiento2016_ELSOC2019.dta")%>%
               rename(idencuesta=folio,pd_diseno=W,pd_nr=W1,pd_sexo=W3,pd_muestra=ponderador1)%>%
              select(idencuesta, pd_diseno,pd_nr,pd_sexo,pd_muestra)%>%             
                mutate(ola=4,
               muestra=1)
...
# COMPARAR CON BASE LONG --------------------------------------------------


elsoc_16 <- elsoc_long_2016_2022%>%
            filter(ola==1)%>%
            select(idencuesta,ponderador02)%>%
            left_join(pond_16,by="idencuesta")


elsoc_17 <- elsoc_long_2016_2022%>%
  filter(ola==2)%>%
  select(idencuesta,ponderador02)%>%
  left_join(pond_17,by="idencuesta")

elsoc_18 <- elsoc_long_2016_2022%>%
  filter(ola==3)%>%
  select(idencuesta,ponderador02)%>%
  left_join(pond_18,by="idencuesta")


elsoc_19 <- elsoc_long_2016_2022%>%
  filter(ola==4)%>%
  select(idencuesta,ponderador02)%>%
  left_join(pond_19,by="idencuesta")


elsoc_21 <- elsoc_long_2016_2022%>%
  filter(ola==5)%>%
  select(idencuesta,ponderador02)%>%
